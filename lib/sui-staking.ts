// Mock Sui client - @mysten/sui@2.3.2 doesn't expose concrete client class
import { createServerClient } from "@/lib/supabase/server"

const suiClient = { url: "https://fullnode.mainnet.sui.io:443" }
...
export async function fetchSuiValidators(): Promise<ValidatorInfo[]> {
  try {
    ...
    return validators.map((validator) => ({
      ...
      stakedSui: validator.stakedSui || "0",
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .upsert({
        ...
        total_stake: validator.stakedSui || "0",
        delegators_count: 0,
