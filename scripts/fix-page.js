...
const target = "/vercel/share/v0-project/app/page.tsx"

const content = `"use client"

import { ArrowRightLeft, TrendingUp, Shield, Zap, Coins, Grid3x3, Activity, AlertTriangle, Gift } from "lucide-react"
...
// =====================
const ACTIVITY = [
  { id: 1, type: "swap",    title: "Swap Executed",          sub: "Via Cetus Protocol",    amount: "-500 SUI",     badge: "+4,200 ATLAS" },
  { id: 2, type: "warning", title: "Scam Contract Blocked",  sub: "Auto-Blocked",          amount: "BLOCKED",      badge: "Wallet Protection" },
  { id: 3, type: "oracle",  title: "Oracle Update: SUI/USD", sub: "Pyth Network · 30s",    amount: "$1.24",        badge: "+4.2%" },
  { id: 4, type: "event",   title: "Whale Inflow",           sub: "Bridge · 12m ago",      amount: "+250,000 SUI", badge: "" },
]
...
const iconComp  = { swap: ArrowRightLeft,        warning: AlertTriangle,     oracle: Zap,             event: Gift               }
          ...
          return (
            <div key={item.id} className="bg-card/50 backdrop-blur-xl border border-border/10 rounded-xl p-4 hover:border-border/40 smooth-transition cursor-pointer">
              <div className="flex items-start gap-3">
                <div className={\`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 \${iconBg[item.type]}\`}>
                  <Ic className={\`h-4 w-4 \${iconColor[item.type]}\`} />
                </div>
                ...
                <div className="text-right flex-shrink-0">
                  <div className={\`text-sm font-semibold \${item.type === "warning" ? "text-red-500" : item.amount.startsWith("+") ? "text-green-500" : "text-foreground"}\`}>{item.amount}</div>
  ...
  return (
    <aside className="space-y-4">
      ...
      <div className="bg-gradient-to-br from-primary/10 to-accent/10 border border-primary/20 rounded-2xl p-4">
        ...
        <div className="text-xs text-muted-foreground mb-2">Next: 150 ATLAS · 2.5x Multiplier</div>
...
console.log("[v0] SUCCESS: page.tsx written, length:", content.length)
