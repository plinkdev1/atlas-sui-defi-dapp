...
import { Check, Download, ExternalLink, Loader2, QrCode, Shield, Key } from "lucide-react"
...
// ReownWalletModal removed - using @mysten/dapp-kit as single wallet system
import { WalletDiscoveryBanner } from "@/components/wallet-discovery-banner"
import { linkWalletToUser } from "@/lib/user-data-service"
...
export function WalletConnectionModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleWalletConnect = (walletName: string) => {
    ...
    connectWallet(
      ...
      {
        onSuccess: () => {
          // Link wallet to supabase user if signed in
          if (supabaseUser && wallet.address) {
            linkWalletToUser(wallet.address, walletName).catch(() => {})
          }
          setConnectingName(null)
          // onClose is handled by the useEffect watching currentAccount
        },
        onError: (err) => {
